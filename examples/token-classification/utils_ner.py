...
def read_teacher_predictions_from_file(file_path, num_examples, max_seq_length):
    teacher_predictions = [[]]
    nbr_examples = 0
    with open(file_path, "r") as f:
        for line in f:
            if len(line) == 0:
                ...
                teacher_predictions[nbr_examples].append([float(x) for x in line.split(" ")])
    # should end up with shape (nbr_examples, max_seq_length, nbr_labels)
    if len(teacher_predictions[nbr_examples]) == 0:
        teacher_predictions = teacher_predictions[:nbr_examples]

    assert len(teacher_predictions) == num_examples
...
class InputFeaturesKD:
    ...
    token_type_ids: Optional[List[int]] = None
    label_ids: Optional[List[int]] = None
    teacher_predictions: Optional[List[List[float]]] = None
...
class TokenClassificationTask:
    ...
    @staticmethod
    def convert_examples_to_features_kd(
        teacher_predictions: List[List[List[float]]],
        examples: List[InputExample],
        label_list: List[str],
        max_seq_length: int,
        tokenizer: PreTrainedTokenizer,
...
if is_torch_available():
    ...
    class TokenClassificationDatasetKD(Dataset):
        """
        This will be superseded by a framework-agnostic approach
        soon.
        """
        ...
        def __init__(
            ...
            cached_features_file = os.path.join(
                data_dir,
                "cached_{}_{}_{}_{}".format(mode.value, tokenizer.__class__.__name__, str(max_seq_length), "kd"),
            )
